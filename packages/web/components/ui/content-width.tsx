...
export function ContentWidth({
    ...
    const [setting, setSetting] = React.useState<ContentWidthSetting>("default");

    React.useEffect(() => {
        try {
            const stored = localStorage.getItem("contentWidthSetting");
            if (stored === "narrow" || stored === "default" || stored === "wide") {
                setSetting(stored);
            }
        } catch {}
    }, []);
    ...
    return (
        <div className={className} style={{ maxWidth, width: "100%" }}>
            {children}
        </div>
    );
}
