...
var exampleTable = pgTable("example", {
...
function createDb(connectionString) {
  return drizzle(Xs(connectionString), { schema: schema_exports });
}
__name(createDb, "createDb");

// src/index.ts
var app = new Hono2().use(cors()).get("/", (c) => {
  ...
  try {
    ...
    return c.json(
      { message: "OK! Server is running. Database connected.", success: true },
      { status: 200 }
    );
  } catch (e) {
    return c.json(
...
// .wrangler/tmp/bundle-n6m04j/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-n6m04j/middleware-loader.entry.ts
