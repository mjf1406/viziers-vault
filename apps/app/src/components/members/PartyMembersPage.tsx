import { SearchIcon, UserMinusIcon, UsersIcon } from "lucide-react";
...
import { usePartyMembers } from "@/hooks/members/usePartyMembers";
import { useRemovePartyMember } from "@/hooks/members/useRemovePartyMember";
...
export function PartyMembersPage({ partyId }: PartyMembersPageProps) {
  ...
  return (
    <div className="flex w-full flex-col gap-6 px-4 py-8 sm:px-8">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight">{t("navMembers")}</h1>
      </div>
      ...
      {showSkeleton ? <MembersSkeleton /> : null}
      ...
      {!showSkeleton && !isError && filtered.length > 0 ? (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filtered.map((member) => {
            ...
            const canRemove = !isSelf && member.role !== "leader";
            return (
              <Card key={member.userId} size="sm">
                <CardHeader className="flex flex-row items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <PartyRoleBadge role={member.role} className="mt-1" />
                  </div>
                </CardHeader>
                {canRemove ? (
                  <CardContent>
