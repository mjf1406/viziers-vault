...
import { worldFormSchema, type WorldFormValues } from "@/lib/worlds/worldFormSchema";
import type { Id } from "../../../convex/_generated/dataModel";

type VisualMode = "fontawesome" | "emoji" | "image";

export type WorldFormCredenzaMode = "create" | "edit";

export type WorldFormInitialValues = {
  name: string;
  description?: string;
  icon?: string;
...
type WorldFormCredenzaProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: WorldFormCredenzaMode;
  initialValues?: WorldFormInitialValues;
  onSubmit: (values: WorldFormValues) => Promise<void>;
};
...
function resolveVisualMode(
  values: Pick<WorldFormInitialValues, "icon" | "imageFileId">,
): VisualMode {
...
export function WorldFormCredenza({
  open,
  onOpenChange,
  mode,
  initialValues,
  onSubmit,
}: WorldFormCredenzaProps) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      setSubmitError(null);
      const parsed = worldFormSchema.parse({
        ...value,
