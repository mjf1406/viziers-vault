...
type SetPartyArchivedArgs = {
  partyId: Id<"parties">;
  archived: boolean;
};

export function useSetPartyArchived() {
  const { t } = useTranslation("parties");
  const { t: tCommon } = useTranslation("common");
  const mutationFn = useConvexMutation(api.parties.setArchived);
  const listKey = partiesListQueryKey();

  return useOptimisticMutation({
    mutationFn: (args: SetPartyArchivedArgs) => mutationFn(args),
