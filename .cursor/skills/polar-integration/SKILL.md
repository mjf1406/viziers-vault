...
- **`theme` (UI hint, not an API field).** The hosted checkout UI accepts `?theme=light|dark` in the URL. To apply a theme, append it to `result.url` _after_ creating the session — it's not a field on `polar.checkouts.create()`.
...
export async function GET(request: Request): Promise<Response> {
  ...
  const json = (key: string) =>
    url.searchParams.has(key) ? JSON.parse(url.searchParams.get(key) ?? "{}") : undefined;
